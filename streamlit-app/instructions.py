...
def guide():
    with st.expander("What is this?🎢"):
        ...
        st.markdown("Under the hood, this app uses [integer programming](https://en.wikipedia.org/wiki/Integer_programming) to solve the following:")
        st.markdown("* **Given some maximum amount of time a user is willing to spend waiting in line to go on rides, maximize the total number of times the user actually goes on a ride, while also respecting certain preferences set by the user**")
        st.markdown("Solving this problem depends on some assumptions made regarding how long it takes to wait in line for any given ride - are the wait-times for a given ride throughout the day **constant**, or do they **dynamically change**?")
        st.markdown("This app solves both cases - see below for the formal description of each")
        st.markdown("**These optimization models applied to real-time data from 3 major California amusement parks, as well as a demo mode for testing things out**")
        st.markdown("* The real-time data comes from: [ThemeParks](https://themeparks.wiki/)")
    ...
    with st.expander("Formal Description Under Constant Time Assumptions"):
    ...
    with st.expander("Formal Description Under Dynamically Changing Time Assumptions"):
        ...
        st.subheader("Variables:")
        st.markdown("Let R be the set of all unique rides $r_i$")
        st.markdown("Let $t_s$ represent the number of time steps")
        st.markdown("Let $T$ represent the set $\{1, \dots, t_s\}$")
        st.markdown("Let $t_d$ represent the duration of each time step")
        st.markdown("Let $t_u$ represent the amount of total time a user wants to spend at the park")
        ...
        st.markdown(" * If $r_{(i, u)}$ = 0, then ride i has 0 ride occurrences")
        st.subheader("Objective:")
        st.latex("\max \sum_{i \in R, t \in \{0, \dots, t_s\}} r_{(i, o, t)}")
        st.markdown("Subject to:")
