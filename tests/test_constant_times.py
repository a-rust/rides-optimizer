...
def test_set_ride_weights():
    ...
    park = ct.OptimizeConstant(rides, wait_times, user_preferences)
    # The ride weight of ride i is the wait time of ride i plus the ride time of ride i
    assert park.set_ride_weights() == {'a': 2, 'b': 4, 'c': 6}
...
def test_maximize_rides_with_max_time():
    ...
    # Choosing to ride 'a' 10 times is optimal, given max_time = 20
    assert park.maximize_rides(ride_weights) == {'a': 10.0, 'b': 0.0, 'c': 0.0}
...
def test_maximize_rides_with_max_ride_repeats():
    ...
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # Given a max ride repeat of 2 and a max time of 30, it is optimal to go on every ride twice
    assert park.maximize_rides(ride_weights) == {'a': 2.0, 'b': 2.0, 'c': 2.0}
...
def test_maximize_rides_with_no_max_ride_repeats():
    ...
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # If no max ride repeats, all of the space will be go to 'a', as the wait time of 'a' is less than the weights of 'b' and 'c'
    assert park.maximize_rides(ride_weights) == {'a': 15.0, 'b': 0.0, 'c': 0.0}

# Test for maximizing the number of rides with required rides constraint
def test_maximize_rides_with_required_rides():
    user_preferences = up.UserPreferences(required_rides=['b'], avoid_rides=None, min_distinct_rides=None, max_ride_repeats=None, max_time=30, min_total_rides=None)
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # Requiring to go on ride 'b' at least once will assign 1 to 'b', and utilize the rest of the space with 'a'
    assert park.maximize_rides(ride_weights) == {'a': 13.0, 'b': 1.0, 'c': 0.0}

# Test for maximizing the number of rides with avoid rides constraint
def test_maximize_rides_with_avoid_rides():
    user_preferences = up.UserPreferences(required_rides=None, avoid_rides=['a'], min_distinct_rides=None, max_ride_repeats=None, max_time=30, min_total_rides=None)
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # Avoiding 'a' entirely will result in utilizing 'b' for all of the space, as the weight of 'b' is less than the weight of 'c'
    assert park.maximize_rides(ride_weights) == {'a': 0.0, 'b': 7.0, 'c': 0.0}
...
def test_maximize_rides_with_no_required_or_avoid_rides():
    ...
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # Not specifying any required rides will result in utilizing all of the space with 'a', as the weight of 'a' is less than the weights of 'b' and 'c'
    assert park.maximize_rides(ride_weights) == {'a': 15.0, 'b': 0.0, 'c': 0.0}
...
def test_maximize_rides_with_min_distinct_rides():
    ...
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # We require all 3 distinct rides, which means we must ride rides 'a', 'b', 'c' at least once. It is optimal to ride 'b' and 'c' once, and allocate the rest of the space to 'a'
    assert park.maximize_rides(ride_weights) == {'a': 10.0, 'b': 1.0, 'c': 1.0}
...
def test_minimize_time_with_min_total_rides():
    ...
    park = ct.OptimizeConstant(rides, wait_times,  user_preferences)
    ride_weights = park.set_ride_weights()
    # Requiring to go on at least 10 total rides (not necessarily distinct) will result in repeating 'a' 10 times. Assigning a positive integer value to min_distinct_rides will force going on more than just 'a' 
    assert park.minimize_time(ride_weights) == {'a': 10.0, 'b': 0.0, 'c': 0.0}
