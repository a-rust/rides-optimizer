...
def test_set_ride_weights():
    ...
    park = dt.OptimizeDynamic(all_rides=rides, time_steps=time_steps, frequency=frequency, wait_times=wait_times, ride_times=ride_times, user_preferences=user_preferences)
...
def test_maximize_no_preferences():
    ...
    # Given a frequency of 100 (i.e., each time step lasts 100 units), it is optimal to ride 'a' 20 times in the first time step, and 'b' 20 times in the second time step. With additional preferences (like max ride repeats), the problem will become more interesting
    assert park.maximize_rides(ride_weights) == {('a', 1): 20.0, ('a', 2): 0.0, ('b', 1): 0.0, ('b', 2): 20.0, ('c', 1): 0.0, ('c', 2): 0.0}
...
def test_maximize_with_required():
    ...
    assert park.maximize_rides(ride_weights) == {('a', 1): 20.0, ('a', 2): 0.0, ('b', 1): 0.0, ('b', 2): 18.0, ('c', 1): 0.0, ('c', 2): 1.0}
...
def test_maximize_with_avoid():
    ...
    assert park.maximize_rides(ride_weights) == {('a', 1): 0.0, ('a', 2): 0.0, ('b', 1): 11.0, ('b', 2): 20.0, ('c', 1): 0.0, ('c', 2): 0.0}
...
def test_maximize_with_max_ride_repeat_constraint():
    ...
    park = dt.OptimizeDynamic(all_rides=rides, time_steps=time_steps, wait_times=wait_times, frequency=frequency, ride_times=ride_times, user_preferences=user_preferences)
    ride_weights = park.set_ride_weights()
    ...
    assert park.maximize_rides(ride_weights) == {('a', 1): 9.0, ('a', 2): 1.0, ('b', 1): 0.0, ('b', 2): 10.0, ('c', 1): 4.0, ('c', 2): 6.0}
